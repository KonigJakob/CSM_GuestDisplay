...
func set_up_ui_elements():
	...
	button_home.position = Vector2(button_home.size.x, get_viewport_rect().size.y - button_home.size.y * 2)
	localization_buttons.position = Vector2(get_viewport_rect().size.x - localization_buttons.size.x - 35, button_home.position.y)
